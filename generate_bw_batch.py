#!/usr/bin/env python3
"""
Batch generate all 23 TCM Way B&W comics + force grayscale via ImageMagick.
Result: single-channel Grayscale PNG, Δ=0.
"""
import subprocess
import os
import sys
...
BAILIAN_DIR = os.path.expanduser("~/bailian-output/images")
...
def find_bl_output():
    """Find the most recently created file in bailian-output/images."""
    if not os.path.isdir(BAILIAN_DIR):
        return None
    files = [os.path.join(BAILIAN_DIR, f) for f in os.listdir(BAILIAN_DIR) if f.endswith('.png')]
    if not files:
        return None
...
def convert_to_grayscale(src_path, dst_path):
    """Convert PNG to single-channel grayscale using ImageMagick."""
    tmp = dst_path + ".gray.png"
    result = subprocess.run(
        ["magick", src_path, "-colorspace", "Gray", tmp],
    ...
    if not os.path.exists(tmp):
        print(f"    No output from ImageMagick")
        return False
    
    # Verify it's grayscale
    verify = subprocess.run(
        ["magick", "identify", "-verbose", tmp],
        capture_output=True, text=True, timeout=10
    )
    if "Type: Grayscale" in verify.stdout or "gray" in verify.stdout.lower():
        os.rename(tmp, dst_path)
        size_kb = os.path.getsize(dst_path) / 1024
        print(f"    ✅ Grayscale verified → {dst_path} ({size_kb:.0f}KB)")
        return True
    else:
        print(f"    ⚠️  Not grayscale, keeping as-is")
        os.rename(tmp, dst_path)
        return True  # still saved


def generate_one(num):
    """Generate one comic: bl CLI → find output → grayscale convert."""
    comic = COMICS[num]
    target = os.path.join(OUT_DIR, f"comic-{num}-bw.png")
    
    # Record existing bailian files to find new one
    before_files = set()
    if os.path.isdir(BAILIAN_DIR):
        before_files = set(os.listdir(BAILIAN_DIR))
    
    print(f"\n📖 #{num}: {comic['title']}")
    print(f"   Prompt: {len(comic['prompt'])} chars")
    
    # Step 1: Generate via bl CLI
    try:
        result = subprocess.run(
            [
                "bl", "image", "generate",
    ...
    if result.returncode != 0:
        print(f"  ❌ bl error: {result.stderr[:300]}")
        return False
    
    # Step 2: Find generated file
    src_path = None
    
    # Check if --output was respected
    if os.path.exists(target):
        src_path = target
        print(f"  → bl wrote to requested path")
    else:
        # Check bailian output directory for new file
        after_files = set(os.listdir(BAILIAN_DIR)) if os.path.isdir(BAILIAN_DIR) else set()
        new_files = after_files - before_files
        if new_files:
            newest = max(new_files, key=lambda f: os.path.getmtime(os.path.join(BAILIAN_DIR, f)))
            src_path = os.path.join(BAILIAN_DIR, newest)
            print(f"  → found in bailian-output: {newest}")
        else:
            # Last resort: parse stdout
            match = re.search(r'(/[^\s]+\.png)', result.stdout)
            if match and os.path.exists(match.group(1)):
                src_path = match.group(1)
                print(f"  → found via stdout: {os.path.basename(src_path)}")
    
    if not src_path or not os.path.exists(src_path):
        print(f"  ❌ Cannot find generated file")
        print(f"  stdout: {result.stdout[:300]}")
        return False
    
    # Step 3: Convert to grayscale
    return convert_to_grayscale(src_path, target)


def main():
    nums = sorted(COMICS.keys())
    total = len(nums)
    success = 0
    failed = []
    
    print("=" * 70)
    print(f"BATCH B&W GENERATION: {total} comics")
    print("Pipeline: bl CLI → ImageMagick -colorspace Gray → Δ=0")
    print("=" * 70)
    ...
    for i, num in enumerate(nums):
        ...
        print(f"  [{i+1}/{total}] {success} ok, {len(failed)} fail | {elapsed:.0f}s elapsed")
    ...
    elapsed = time.time() - start_time
    print("\n" + "=" * 70)
    print(f"COMPLETE: {success}/{total} generated in {elapsed:.0f}s ({elapsed/60:.1f}min)")
    if failed:
        print(f"FAILED: {', '.join(failed)}")
    else:
        print("🎉 ALL 23 B&W COMICS GENERATED SUCCESSFULLY")
    print("=" * 70)
